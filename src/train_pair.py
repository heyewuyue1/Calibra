...
def make_qs_pairwise_data(raw_x):
    qs_x = []
    qs_x1 = []
    qs_x2 = []
    qs_y = []
    ...
    for x in raw_x:
        if x["query_stages"] != {}:
            for stage_name, stage_info in x["query_stages"].items():
                if stage_info[0] == 1 and (eval(stage_info[1]), eval(stage_info[2])) not in qs_y:
                    qs_tree = [Node('Root', 0, [], -1, -1)]
                    qs_x.append(encoder.featurize(preprocessor.query_stage2tree(qs_tree, 1, 1, stage_info[3])))
                    qs_y.append((eval(stage_info[1]), eval(stage_info[2])))
    for i in range(len(qs_y)):
        for j in range(i + 1, len(qs_y)):
            if qs_y[i][0] > qs_y[j][0] and qs_y[i][1] > qs_y[j][1]:
                qs_x1.append(qs_x[i])
                qs_x2.append(qs_x[j])
                qs_labels.append(1)
            elif qs_y[i][0] < qs_y[j][0] and qs_y[i][1] < qs_y[j][1]:
                qs_x1.append(qs_x[i])
                qs_x2.append(qs_x[j])
                qs_labels.append(0)
    return qs_x1, qs_x2, qs_labels
...
if __name__ == "__main__":
    ...
    train_qs_x1, train_qs_x2, train_qs_y = make_qs_pairwise_data(raw_train_x)
    logger.info(f"Number of pair-wise query stages (Train): {len(train_qs_y)}")
    val_qs_x1, val_qs_x2, val_qs_y = make_qs_pairwise_data(raw_val_x)
    logger.info(f"Number of pair-wise query stages (Validation): {len(val_qs_y)}")
    ...
    train_qs_y = torch.tensor(train_qs_y, dtype=torch.float32).view(-1, 1).to(device)
    val_qs_y = torch.tensor(val_qs_y, dtype=torch.float32).view(-1, 1).to(device)

    if not TrainConfig.inference_only:
        num_batches = (len(train_plan_x) + TrainConfig.batch_size - 1) // TrainConfig.batch_size
        qs_batch_size = (train_qs_y.shape[0] + num_batches - 1) // num_batches
        ...
        for epoch in range(TrainConfig.epochs):
            ...
            qs_perm = torch.randperm(len(train_qs_y))
            train_qs_x1 = [train_qs_x1[i] for i in qs_perm]
            train_qs_x2 = [train_qs_x2[i] for i in qs_perm]
            train_qs_y = train_qs_y[qs_perm]
            ...
            for b in range(num_batches):
                ...
                start_qs_idx = b * qs_batch_size
                end_qs_idx = min((b + 1) * qs_batch_size, len(train_qs_y))
                batch_qs_x1 = train_qs_x1[start_qs_idx:end_qs_idx]
                batch_qs_x2 = train_qs_x2[start_qs_idx:end_qs_idx]
                batch_qs_y = train_qs_y[start_qs_idx:end_qs_idx]

                flattened_trees = flatten_tree_batch(batch_x)
                pred = model(flattened_trees)
                mse_loss = mse_loss_fn(pred, batch_y)

                flattened_qs_x1 = flatten_tree_batch(batch_qs_x1)
                flattened_qs_x2 = flatten_tree_batch(batch_qs_x2)
                pred_1 = model(flattened_qs_x1)
                pred_2 = model(flattened_qs_x2)
                diff = pred_1 - pred_2
                prob_y = sigmoid(diff)
                bce_loss = bce_loss_fn(prob_y, batch_qs_y)
                loss = mse_loss + TrainConfig.bce_loss_weight * bce_loss

                optimizer.zero_grad()
                loss.backward()
                optimizer.step()

                logger.info(f"Batch {b+1}/{num_batches}, Train Loss (MSE) = {mse_loss.item():.2f}, Train Loss (BCE) = {bce_loss.item():.2f}, Train Loss = {loss.item():.2f}")
                train_loss_history.append(loss.item())
                global_step = epoch * num_batches + b + 1  # 全局 step
                writer.add_scalar("Loss/Train(MSE)", mse_loss.item(), global_step)
                writer.add_scalar("Loss/Train(BCE)", bce_loss.item(), global_step)
