...
object JoinReorderDP extends PredicateHelper with Logging {
  ...
  private def searchLevel(
    ...
    val pruneMode = conf.getConfString("spark.sql.cbo.joinReorder.dp.pruneMode",
      "none")
    if (calibraEnabled && pruneMode != "none") {
      val pruneLimit = conf.getConfString("spark.sql.cbo.joinReorder.dp.pruneLimit",
        "4").toInt
      prunePlans(nextLevel, pruneMode, pruneLimit)
    }
    nextLevel
  }

  private def prunePlans(nextLevel: JoinPlanMap, mode: String, k: Int): JoinPlanMap = {
    val all = nextLevel.toList
    if (all.length <= k) {
      return nextLevel
    }
    if (mode == "random") {
      ...
      val dropKeys = rand.shuffle(all).take(toDrop).map(_._1)
      dropKeys.foreach { key =>
        nextLevel.remove(key)
      }
      logWarning(s"[DP-Prune] pruned ${dropKeys.size} / ${all.size} plans in this level")
      nextLevel
    } else if (mode == "top") {
      val joinPlans = all.map(_._2)
      val costs: List[Double] = CalibraClient.chooseBestPlan(0, joinPlans)
      val keyCostList = all.map(_._1).zip(costs)
      val topKeys = keyCostList.sortBy(_._2).take(k).map(_._1).toSet
      nextLevel.keys.filterNot(topKeys.contains).foreach(nextLevel.remove)
      logWarning(s"[DP-Prune] pruned ${all.size - topKeys.size} / " +
  ...
  case class JoinPlan(
    ...
    def betterThan(other: JoinPlan, conf: SQLConf): Boolean = {
      ...
      if (other.planCost.card == 0 || other.planCost.size == 0) {
        ...
        original = Math.pow(relativeRows.doubleValue, conf.joinReorderCardWeight) *
      ...
      if (globalCalibraEnabled && joinCount < calibraThreshold) {
