  ...
  object JoinSelection extends Strategy with JoinSelectionHelper {
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case j @ ExtractEquiJoinKeys(joinType, leftKeys, rightKeys, nonEquiCond,
          _, left, right, hint) =>
        def createBroadcastHashJoin(onlyLookingAtHint: Boolean, force: Boolean = false) = {
          // 确定 build side
          val buildSide: Option[BuildSide] = if (force) {
            Some(getSmallerSide(left, right))  // 强制选择较小的一侧
          } else {
            val bs = getBroadcastBuildSide(left, right, joinType, hint, onlyLookingAtHint, conf)
            checkHintBuildSide(onlyLookingAtHint, bs, joinType, hint, true)
            bs  // 本身就是 Option[BuildSide]
          }
          buildSide.map {
            buildSide =>
        ...
        def canConservativelyForceBHJ(
          ...
          val lower = threshold
          val upper = (threshold * (1.0 + forceBHJMargin)).toLong
          ...
          val smallerSide = {
            if (leftSize < rightSize) leftSize
            else rightSize
          }

          smallerSide > lower && smallerSide <= upper
        }
        ...
        def createJoinWithoutHint() = {
          val defaultJoin = createBroadcastHashJoin(false)
            ...
            if (greedyEnabled && calibraEnabled &&
              ...
              !defaultJoin.exists(_.isInstanceOf[joins.BroadcastHashJoinExec]) &&
              canConservativelyForceBHJ(left, right, conf)) {
              // 强制生成一个 BHJ
              val forcedBHJ = createBroadcastHashJoin(onlyLookingAtHint = false, force = true)
              logWarning(s"Forcing BroadcastHashJoinExec for join: ${plan.toString}")
              defaultJoin ++ forcedBHJ.get
            } else {
              defaultJoin
            }
        }
