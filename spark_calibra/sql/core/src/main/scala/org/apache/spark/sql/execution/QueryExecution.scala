...
//    planner.plan(ReturnAnswer(plan)).next()
    ...
    candidates.foreach { p =>
      val logicalPlan = plan
      p.setLogicalLink(logicalPlan)
    }
    val costs = CalibraClient.chooseBestPlan(1, candidates.toList)
